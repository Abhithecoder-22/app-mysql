...
router.route('/delivery')
    .get(function (req, res, next) {
        ...
        // if choose from existing address => redirect
        // if create new address
        // 1. Open form
        // 2. Save data
        // 3. Redirect
    });
...
router.route('/review')
    .get(function (req, res, next) {
        ...
        // Calculate GST
        var gstAmount = req.session.cartSummary.subTotal * gstRate;
...
router.route('/invoice')
    .get(function (req, res, next) {
        // Add your logic to fetch invoice data or generate the invoice

        // Example: Fetch invoice data from the database
        // var invoiceId = req.query.invoiceId; // Assuming you pass the invoiceId as a query parameter
        // var selectQuery = 'SELECT * FROM invoices WHERE id = ' + invoiceId;

        // RunQuery(selectQuery, function (rows) {
        // if (rows.length > 0) {
        // var invoice = rows[0];
        // Pass the invoice data to the invoice.jade template for rendering
